...
class Config:
    ...
    # Low-light Input or Enhancement Comparision Methods
    exp_name: str = "variance"
    # exp_name: str = "low"
...
def create_splats_with_optimizers(
    ...
    # Basic curve (1D look-up table)
    #LUT_t = torch.nn.Parameter(torch.linspace(0, 1, 200))   # An Learnable Curve
    #params.append(("curve", torch.nn.Parameter(LUT_t), 2.5e-3 / 10))
...
class Runner:
    ...
    def __init__(self, cfg: Config) -> None:
        ...
        print("training set length", len(self.trainset))    # Training View Numbers
        # Model
        ...
        # print("The learnable curve dims", self.splats["curve"].shape[-1])
    ...
    def rasterize_splats(
        ...
        **kwargs,
        ...
        return render_colors, render_alphas, info   # return colors and alphas

    def train(self):
        cfg = self.cfg
        device = self.device

        # curve = self.splats["curve"]    # [dims]
        ...
        #print('000 the length of iters', len(trainloader_iter))
        trainloader_iter = iter(trainloader)
        #print('111 the length of iters', len(trainloader_iter))
        ...
        for step in pbar:
            ...
            #print('the image ids is', image_ids)
            ...
            # forward
            renders_low, alphas, info = self.rasterize_splats(
            ...
            if renders_low.shape[-1] == 4:
                colors, depths = renders_low[..., 0:3], renders_low[..., 3:4]
            else:
                colors, depths = renders_low, None
            ...
            # colors: predict result, pixels: GT
    ...
    def eval(self, step: int):
        ...
        for i, data in enumerate(valloader):
            ...
            imageio.imwrite(
                f"{self.render_dir_depth}/val_{i:04d}_depth.png", (depth.squeeze(0).squeeze(-1).cpu().numpy() * 255).astype(np.uint8)
            )

            imageio.imwrite(
                f"{self.render_dir}/val_{i:04d}_pred.png", (colors.squeeze(0).cpu().numpy() * 255).astype(np.uint8)
            )

            imageio.imwrite(
                f"{self.render_dir}/val_{i:04d}_gt.png", (pixels.squeeze(0).cpu().numpy() * 255).astype(np.uint8)
            )
    ...
    def render_traj(self, step: int):
        ...
        #camtoworlds = self.parser.camtoworlds[8:15]
        #camtoworlds = generate_interpolated_path(camtoworlds, 20)  # [N, 3, 4]
